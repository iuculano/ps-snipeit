...
function Get-SnipeITModel
{
    ...
    Process
    {
        switch ($PSCmdlet.ParameterSetName)
        {
            ...
            "Id"
            {
                $endpoint = "$Url/api/v1/models/$Id"
                break
            }
        }
    

        Invoke-SnipeITRestMethod -Method "GET" -Url $endpoint -APIKey $APIKey | ConvertFrom-SnipeITAPI -Type "axSnipeIT.Model"
    }
}
