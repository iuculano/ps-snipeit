<#
    .SYNOPSIS
    Gets one or more Snipe-IT locations.

    .PARAMETER Id
    Specifies one or more locations by id.
...
function Get-SnipeITLocation
{
    ...
    Process
    {
        switch ($PSCmdlet.ParameterSetName)
        {
            ...
            "Id"
            {
                $endpoint = "$Url/api/v1/locations/$Id"
                break
            }
        }
    

        Invoke-SnipeITRestMethod -Method "GET" -Url $endpoint -APIKey $APIKey | ConvertFrom-SnipeITAPI -Type "axSnipeIT.Location"
    }
}
